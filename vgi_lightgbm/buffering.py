...
Buffering functions (model fit, cross_val_predict, cross_val_score, grid_search)
need the whole input before producing output. The sink phase serializes each
input batch to execution-scoped storage; finalize reassembles the full table.
This module holds the serialization, storage, and (numeric) matrix-assembly
helpers plus the single-bucket sink/combine implementation so each function only
writes its finalize logic.

LightGBM accepts categorical (string) columns directly, so most feature handling
lives in ``features.py``; ``matrix`` here is the all-numeric fast path used by
the synthetic datasets and the numeric-only helpers.
"""

...
#
# Dispatch the single vgi-lightgbm image into one of its two transports:
...
#   /data/models  -> LIGHTGBM_MODELS_DIR  (model registry)
set -e
...
mkdir -p "${LIGHTGBM_MODELS_DIR:-/data/models}"
...
case "${1:-http}" in
  ...
  stdio)
    shift 2>/dev/null || true
    exec python /app/lightgbm_worker.py "$@"
    ;;
  *)
    exec "$@"
    ;;

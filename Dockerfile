...
#
# Single image that serves BOTH transports of the LightGBM VGI worker:
#   docker run ... IMG            -> HTTP server on $PORT (default, Fly.io / local)
#   docker run -i ... IMG stdio   -> stdio worker DuckDB spawns on-host
# See docker-entrypoint.sh. Deps install from PyPI (vgi-python / vgi-rpc / lightgbm
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-lightgbm
...
#           model registry)
LABEL org.opencontainers.image.title="vgi-lightgbm" \
      org.opencontainers.image.description="LightGBM as a VGI worker for DuckDB/SQL (stdio + HTTP)" \
      org.opencontainers.image.source="${SOURCE_URL}" \
      org.opencontainers.image.version="${VERSION}" \
      org.opencontainers.image.revision="${GIT_COMMIT}" \
      org.opencontainers.image.licenses="MIT" \
      farm.query.vgi.volumes='[{"path":"/data","name":"vgi_lightgbm_state","purpose":"state","shared":true}]'

ENV PYTHONUNBUFFERED=1 \
    PIP_NO_CACHE_DIR=1 \
    PIP_DISABLE_PIP_VERSION_CHECK=1 \
    # The local-disk model registry persists under the /data volume (see label).
    LIGHTGBM_MODELS_DIR=/data/models \
    ...
    VGI_LIGHTGBM_GIT_COMMIT=${GIT_COMMIT} \
    SENTRY_RELEASE=${GIT_COMMIT}

WORKDIR /app

# libgomp1 backs LightGBM's OpenMP runtime; curl backs the HEALTHCHECK below.
RUN apt-get update \
